...
const APP_VERSION = "v10.0"; // bump this string on every update to force tutorial reset
...
const TH = {
  IMP:{bg:"#000",s1:"#0d0d0d",s2:"#111",s3:"#161616",b1:"#1c1c1c",b2:"#282828",
    ...
    practiceLabel:"WARRIOR'S PRACTICE",practiceDesc:"Iaido · Kyudo · Systema · Daggers",
    practiceItems:["Nourish","Warrior's Practice","Hydration","Sleep"],
    mindItems:["Morning Declaration","Doctrine","The Book","Vel'nar","Shadow Work","Mood"],
    soulItems:["The Book","Rites","Partnership","Holy Days","Evening Inventory"],
    scoreLabel:"OPERATIONAL CAPACITY",
  },
  TEND:{bg:"#080504",s1:"#100c0a",s2:"#171008",s3:"#1e1510",b1:"#231810",b2:"#2e2016",
    ...
    practiceLabel:"KEEPER'S PRACTICE",practiceDesc:"Yoga · Strength · Walking · Stillness",
    practiceItems:["Nourish","Keeper's Practice","Hydration","Sleep"],
    mindItems:["Morning Declaration","Doctrine","The Book","Shadow Work","Mood"],
    soulItems:["The Book","Rites","Partnership","Holy Days","Morning Quiet","Replenishment","Evening Inventory"],
    scoreLabel:"RESERVOIR LEVEL",
  }
};
...
// ── PROGRESSION SYSTEM ───────────────────────────────────────────────────
// Gates: content unlocks via planner completion + phase assessment.
// Assessment types by content category (based on memory retention research):
//   RECALL     — active recall question, typed answer, minimum 20 words
//                Best for: doctrine text, function names, axioms, language vocab
//                Science: retrieval practice > rereading for long-term retention
//   REFLECTION — open reflection prompt, no wrong answer, minimum 30 words
//                Best for: shadow work, evening inventory, declaration meaning
//                Science: elaborative interrogation deepens schema integration
//   STREAK     — N consecutive planner completions required, no written test
//                Best for: physical practice gates, hydration habits
//                Science: habit formation requires behavioral repetition before
//                         content complexity is added
//   APPLY      — describe one real application from the past 7 days (min 40 words)
//                Best for: unlocking relational content (partnership, rites)
//                Science: transfer-appropriate processing — test matches intended use
//   RITE       — Rite completion attestation required (date + confirmation)
//                Best for: The Book, Holy Days, Language practice, Soul deep content
//                Science: threshold events create episodic memory anchors that
//                         make subsequent content more durable

const PHASE_GATES = {
  // Phase 1 — immediately available day 1
  phase1: {
    label: "Foundation",
    unlocked: true,
    content: ["declaration","planner","hydration","mood","nourish","grocery"],
    assessment: null,
  },

  // Phase 2 — unlocks after 7 completed planner days (not necessarily consecutive)
  phase2: {
    label: "Practice",
    unlocked: false,
    requiredPlannerDays: 7,
    assessmentType: "RECALL",
    assessmentPrompt: {
      IMP: "Write from memory: what is the Axiom of the Uncrowned, and what does each word mean to you in practice?",
      TEND: "Write from memory: what is the Axiom of the Unspent, and what does each word mean to you in practice?",
    },
    minWords: 20,
    content: ["doctrine","evening_inventory","morning_declaration_full"],
  },

  // Phase 3 — unlocks after 14 completed planner days + Phase 2 assessment passed
  phase3: {
    label: "Integration",
    unlocked: false,
    requiredPlannerDays: 14,
    requiresPhase: 2,
    assessmentType: "REFLECTION",
    assessmentPrompt: {
      IMP: "Describe one moment in the past two weeks where the dominant function (Ni) and the inferior function (Se) were in direct conflict. What happened and what did you do?",
      TEND: "Describe one moment in the past two weeks where the dominant function (Fe) pulled you past your own limit. What did it cost you and what did you do about it?",
    },
    minWords: 30,
    content: ["shadow_work","mood_history","supplement_stack"],
  },

  // Phase 4 — unlocks after 30 completed planner days + 7-day practice streak + Phase 3 passed
  phase4: {
    label: "Depth",
    unlocked: false,
    requiredPlannerDays: 30,
    requiredPracticeStreak: 7,
    requiresPhase: 3,
    assessmentType: "APPLY",
    assessmentPrompt: {
      IMP: "Describe one real situation from the past week where you applied the doctrine — not thought about it, but acted from it. Be specific about what you did differently.",
      TEND: "Describe one real situation from the past week where you maintained the limit without apologizing for it. What happened before and after?",
    },
    minWords: 40,
    content: ["holy_days","partnership","communication_translator","velnar_basic"],
  },

  // Phase 5 — unlocks after Rite completion
  phase5: {
    label: "The Rite",
    unlocked: false,
    requiresRite: true,
    assessmentType: "RITE",
    assessmentPrompt: {
      IMP: "Record the date of your Rite completion and confirm: the Oath was spoken aloud, the sigil was marked, the record was written.",
      TEND: "Record the date of your Rite completion and confirm: the Oath was spoken aloud, the anointing was completed, the Rite record was written.",
    },
    content: ["the_book","holy_days_full","velnar_full","rites_complete","soul_deep"],
  },
};

// Content→phase map (what phase is required to access each content item)
const CONTENT_PHASES = Object.entries(PHASE_GATES).reduce((acc, [phase, gate]) => {
  gate.content.forEach(item => { acc[item] = parseInt(phase.replace("phase","")) ; });
  return acc;
}, {});

// ── PROGRESSION HOOK — stub for Layers 5–7 ───────────────────────────────
// In Layer 5+, this hook reads from SQLite (planner_completions, assessments,
// streak_log, rite_log) and returns the current phase and gate status.
// In Layer 2, it returns phase 1 unlocked only (no gate logic yet).
function useProgression(profile) {
  // TODO Layer 5: read plannerDaysCompleted, practiceStreak, assessmentsPassed,
  //              riteCompleted from SQLite
  // For now: return phase 1 only
  return {
    currentPhase: 1,
    isUnlocked: (contentKey) => PHASE_GATES.phase1.content.includes(contentKey),
    gateStatus: PHASE_GATES,
    plannerDaysCompleted: 0,  // TODO: count from SQLite
    assessmentPending: null,  // TODO: check if phase assessment is due
    submitAssessment: (phase, text, profile) => {
      // TODO Layer 5: validate word count, store to SQLite, unlock next phase
      console.log("Assessment submitted for phase", phase, "— implement in Layer 5");
    },
  };
}

// ── PHASE GATE UI COMPONENT — stub ───────────────────────────────────────
// Rendered in place of locked content sections.
// In Layer 2: not rendered (all phase 1 content is accessible).
// In Layer 5+: renders lock state, progress toward unlock, and assessment modal.
function PhaseGate({ contentKey, currentPhase, children }) {
  const requiredPhase = CONTENT_PHASES[contentKey] || 1;
  if (currentPhase >= requiredPhase) return children;
  const gate = PHASE_GATES[`phase${requiredPhase}`];
  // TODO Layer 5: render proper lock UI with progress and assessment trigger
  return null; // Phase gate renders nothing until Layer 5 implements UI
}
...
export default function App() {
  ...
  const [streaks,setStreaks] = useState({planner:3,hydration:5,practice:1,mood:7});
  const [bodyTab,setBodyTab] = useState(0);
  const [mindTab,setMindTab] = useState(0);
  const [soulTab,setSoulTab] = useState(0);
  const [batchDismissed,setBatchDismissed] = useState(false);
  const [otSheet,setOtSheet] = useState(false);
  const [quietSecs,setQuietSecs] = useState(null);
  const [quietRunning,setQuietRunning] = useState(false);
  const [notifDots,setNotifDots] = useState({mind:true,body:true,soul:false,planner:false});
  ...
  // Derived home values
  const tomorrowIdx2 = new Date().getDay(); // 0=Sun
  const tomorrowPlan = (WEEK_PLAN[0].days && WEEK_PLAN[0].days[tomorrowIdx2]) || null;
  const isBatchEve = !!(tomorrowPlan && tomorrowPlan.batch);
  const tomorrowDinner = tomorrowPlan ? tomorrowPlan.meals.d : "";
  const planPct = ITEMS.length ? done_count / ITEMS.length : 0;
  const hydPctRaw = Math.min(1, (hyd[prof] || 0) / T.hydT);
  const score = Math.round(((planPct + hydPctRaw + (moodScore/10)) / 3) * 100);
  ...
  useEffect(()=>{
    if(!quietRunning||quietSecs===null)return;
    if(quietSecs<=0){setQuietRunning(false);setQuietSecs(null);showT("Session complete");setDone(d=>({...d,quiet:true}));return;}
    const t=setInterval(()=>setQuietSecs(s=>s-1),1000);
    return()=>clearInterval(t);
  },[quietRunning,quietSecs]);

  const NAV=[
    {k:"home",   ic:IC.home,  l:"Home",    col:T.gold,  dot:false},
    {k:"planner",ic:IC.plan,  l:"Planner", col:T.cPlan, dot:notifDots.planner},
    {k:"mind",   ic:IC.brain, l:"Mind",    col:T.cMind, dot:notifDots.mind},
    {k:"body",   ic:IC.heart, l:"Body",    col:T.cBody, dot:notifDots.body},
    {k:"soul",   ic:IC.star,  l:"Soul",    col:T.cSoul, dot:notifDots.soul},
  ];
  ...
  return (
    <div style={{minHeight:"100vh",background:"#050505",display:"flex",flexDirection:"column",alignItems:"center",padding:"14px 12px 48px",fontFamily:JOS}}>
      ...
      <div style={{width:370,height:740,background:T.bg,borderRadius:46,border:"9px solid #181818",boxShadow:"0 0 0 1.5px #2a2a2a,0 40px 80px rgba(0,0,0,0.9)",display:"flex",flexDirection:"column",overflow:"hidden",color:T.text,position:"relative"}}>
        ...
        <div style={{flex:1,overflowY:"auto",background:T.bg}}>
          ...
          {/* ══ HOME ══════════════════════════════════════════ */}
            ...
            <div style={{padding:"14px"}}>
              {/* Shift strip */}
              <div style={{background:imp?"#000":T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"9px 13px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{flex:1,cursor:"pointer"}} onClick={()=>{goNav("planner");setPlanTab("schedule");}}>
                  ...
                  <div style={{display:"flex",gap:10}}>
                    {sw&&<span style={{fontSize:7.5,color:T.t3}}>Wake {sw.wk} · Sleep {sw.sl}</span>}
                  </div>
                </div>
                {otVal&&<span style={{fontSize:9,color:"#c07040",marginRight:8}}>OT +{otVal}h</span>}
                <button onClick={()=>setOtSheet(true)} style={{background:"transparent",border:`1px solid ${T.b1}`,borderRadius:3,padding:"3px 8px",color:T.t3,fontSize:7,letterSpacing:"0.1em",cursor:"pointer"}}>{otVal?"EDIT OT":"SET OT"}</button>
              </div>

              {/* Quote */}
              <div style={{background:T.s1,borderLeft:`3px solid ${T.gold}`,borderRadius:"0 8px 8px 0",padding:"9px 13px",marginBottom:8}}>
                <div style={{fontFamily:COR,fontSize:13,color:T.t2,fontStyle:"italic",lineHeight:1.6}}>"{T.quote}"</div>
              </div>

              {/* Quick Actions */}
              <div style={{display:"flex",gap:6,marginBottom:8,overflowX:"auto",paddingBottom:2}}>
                {[
                  {l:"+8oz",   action:()=>{setHyd(h=>({...h,[prof]:Math.min(h[prof]+8,T.hydT)}));showT("+8 oz");}},
                  {l:"Mood",   action:()=>{setShowMood(true);setMoodTab("log");setNotifDots(d=>({...d,mind:false}));}},
                  {l:"✓ Done", action:()=>{const nx=ITEMS.find(it=>!done[it.id]);if(nx){setDone(d=>({...d,[nx.id]:true}));showT("Done");}else showT("All done!");}},
                  {l:"Grocery",action:()=>{goNav("body");setBodyTab(0);}},
                  imp?{l:"Set OT",action:()=>setOtSheet(true)}:{l:"Quiet",action:()=>{setQuietSecs(15*60);setQuietRunning(true);goNav("soul");setSoulTab(4);}},
                ].map((q,i)=>(
                  <button key={i} onClick={q.action} style={{flexShrink:0,padding:"5px 12px",background:T.s2,border:`1px solid ${T.b2}`,borderRadius:20,color:T.t2,fontSize:8.5,letterSpacing:"0.08em",cursor:"pointer",whiteSpace:"nowrap"}}>{q.l}</button>
                ))}
              </div>

              {/* Sovereignty Score / Reservoir Level */}
              <div style={{background:T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"10px 13px",marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                  <span style={{fontSize:7.5,color:T.t3,letterSpacing:"0.12em",textTransform:"uppercase"}}>{T.scoreLabel}</span>
                  <span style={{fontSize:9,color:score>=70?T.teal:score>=40?T.gold:T.t3}}>{score}%</span>
                </div>
                <div style={{background:T.s3,borderRadius:4,height:5,overflow:"hidden"}}>
                  <div style={{width:`${score}%`,height:"100%",background:score>=70?T.teal:score>=40?T.gold:"#6a4a4a",transition:"width 0.5s",borderRadius:4}}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
                  <span style={{fontSize:7,color:T.t3}}>Plan {done_count}/{ITEMS.length}</span>
                  <span style={{fontSize:7,color:T.t3}}>Hyd {hydPct}%</span>
                  <span style={{fontSize:7,color:T.t3}}>Mood {moodScore}/10</span>
                </div>
              </div>

              {/* Partner card */}
              <div style={{background:T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"10px 13px",marginBottom:8,cursor:"pointer"}} onClick={()=>{setShowMood(true);setMoodTab("partner");}}>
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:5}}>
                  <div style={{width:28,height:28,borderRadius:"50%",background:`rgba(${imp?"196,120,120":"201,168,76"},0.1)`,border:`1px solid ${T.partnerCol}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <span style={{fontFamily:COR,fontSize:13,color:T.partnerCol}}>{T.partnerInit}</span>
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:8,color:T.t3}}>{T.partnerName} · synced 2m ago</div>
                    <div style={{display:"flex",gap:3,marginTop:2}}>{[1,2,3,4,5].map(d=><div key={d} style={{width:6,height:6,borderRadius:"50%",background:d<=4?T.teal:T.b2}}/>)}</div>
                  </div>
                  <span style={{fontSize:8,color:T.t3}}>5/8 done</span>
                </div>
                <div style={{fontFamily:COR,fontSize:12,color:T.t2,fontStyle:"italic",lineHeight:1.5,paddingLeft:38}}>{imp?"She's steady today. Low-key evening preferred.":"He's solid. Good capacity. Connection welcome."}</div>
              </div>

              {/* Streak rack */}
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:8}}>
                {(imp?[
                  {k:"planner",  ic:IC.check,l:"DAYS"},
                  {k:"hydration",ic:IC.drop, l:"HYD"},
                  {k:"practice", ic:IC.star, l:"WARRIOR"},
                  {k:"mood",     ic:IC.heart,l:"MOOD"},
                ]:[
                  {k:"planner",  ic:IC.check,l:"DAYS"},
                  {k:"hydration",ic:IC.drop, l:"HYD"},
                  {k:"practice", ic:IC.star, l:"KEEPER"},
                  {k:"mood",     ic:IC.heart,l:"MOOD"},
                ]).map(s=>(
                  <div key={s.k} style={{background:T.s1,border:`1px solid ${streaks[s.k]>0?T.gold+"44":T.b1}`,borderRadius:8,padding:"8px 4px",textAlign:"center"}}>
                    <Svg path={s.ic} size={13} color={streaks[s.k]>0?T.gold:T.t3}/>
                    <div style={{fontFamily:COR,fontSize:18,color:streaks[s.k]>0?T.gold:T.t3,lineHeight:1.1,marginTop:2}}>{streaks[s.k]}</div>
                    <div style={{fontSize:6.5,color:T.t3,letterSpacing:"0.06em",marginTop:1}}>{s.l}</div>
                  </div>
                ))}
              </div>

              {/* Batch cook banner */}
              {isBatchEve&&!batchDismissed&&(
                <div style={{background:`rgba(${imp?"201,168,76":"196,120,120"},0.08)`,border:`1px solid ${T.gold}44`,borderRadius:8,padding:"10px 13px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={()=>{goNav("body");setBodyTab(0);}}>
                  <div>
                    <div style={{fontSize:8.5,color:T.gold,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:2}}>BATCH COOK TOMORROW</div>
                    <div style={{fontSize:12,color:T.text}}>{tomorrowDinner}</div>
                    <div style={{fontSize:8.5,color:T.t3}}>Prep tonight if marinating.</div>
                  </div>
                  <button onClick={(e)=>{e.stopPropagation();setBatchDismissed(true);}} style={{background:"transparent",border:"none",color:T.t3,cursor:"pointer",fontSize:18,lineHeight:1}}>×</button>
                </div>
              )}

              {/* Axiom */}
              <div style={{background:T.glow,border:`1px solid ${T.b1}`,borderRadius:8,padding:"12px",textAlign:"center"}}>
                <div style={{fontFamily:COR,fontSize:14,color:T.text,letterSpacing:"0.14em",textTransform:"uppercase",lineHeight:1.8,whiteSpace:"pre-line"}}>{T.axiom}</div>
              </div>
            </div>
          )}
          ...
          {nav==="mind"&&(
            <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
              {/* Secondary tab bar */}
              <div style={{display:"flex",borderBottom:`1px solid ${T.b1}`,background:imp?"#000":T.s1,overflowX:"auto",flexShrink:0}}>
                {T.mindItems.map((lbl,i)=>(
                  <button key={i} onClick={()=>setMindTab(i)} style={{flexShrink:0,padding:"10px 12px",background:"none",border:"none",borderBottom:`2px solid ${mindTab===i?T.cMind:"transparent"}`,color:mindTab===i?T.cMind:T.t3,fontSize:8,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer"}}>
                    {lbl}
                  </button>
                ))}
              </div>
              <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
                ...
                {mindTab===0&&(
                  <div>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cMind,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Morning Declaration</div>
                    <div style={{background:T.s1,border:`1px solid ${T.cMind}33`,borderRadius:8,padding:"16px",marginBottom:10}}>
                      <div style={{fontFamily:COR,fontSize:16,color:T.text,lineHeight:1.8,marginBottom:8}}>{T.decl}</div>
                      <div style={{fontSize:10,color:T.t3,fontStyle:"italic"}}>{T.declSub}</div>
                    </div>
                    <button onClick={()=>{setDone(d=>({...d,decl:true}));showT("Declaration spoken");}} style={{width:"100%",padding:"11px 0",background:done.decl?T.tealBg:"transparent",border:`1px solid ${done.decl?T.teal:T.b2}`,color:done.decl?T.teal:T.t3,fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",borderRadius:5}}>
                      {done.decl?"✓ Spoken today":"Mark as spoken"}
                    </button>
                  </div>
                )}
                {/* Doctrine stub */}
                {mindTab===1&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.brain} size={40} color={T.cMind}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cMind,letterSpacing:"0.14em",textTransform:"uppercase"}}>Doctrine</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Axioms · Creed · Manifesto · Litany<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* The Book stub */}
                {mindTab===2&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.star} size={40} color={T.cMind}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cMind,letterSpacing:"0.14em",textTransform:"uppercase"}}>The Book</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>The Eight Who Carry the Fire<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* Language stub (Imp only has Vel'nar) */}
                {imp&&mindTab===3&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.star} size={40} color={T.cMind}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cMind,letterSpacing:"0.14em",textTransform:"uppercase"}}>Vel'nar</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>The sovereign language<br/>Vocal coach · Coming in a future layer.</div>
                  </div>
                )}
                {/* Shadow Work */}
                {((imp&&mindTab===4)||(!imp&&mindTab===3))&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.brain} size={40} color={T.cMind}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cMind,letterSpacing:"0.14em",textTransform:"uppercase"}}>Shadow Work</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Eight positions · Integration<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* Mood tracker */}
                {((imp&&mindTab===5)||(!imp&&mindTab===4))&&(
                  <div>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cMind,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Mood</div>
                    <button onClick={()=>{setShowMood(true);setMoodTab("log");}} style={{width:"100%",background:T.s1,border:`1px solid ${T.cMind}33`,borderRadius:8,padding:"14px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",marginBottom:8,textAlign:"left"}}>
                      <div><div style={{fontSize:14,color:T.text,marginBottom:2}}>Log Today's Mood</div><div style={{fontSize:10,color:T.t3}}>Auto-translates via Groq · shares with {T.partnerName}</div></div>
                      <span style={{color:T.cMind,fontSize:18}}>›</span>
                    </button>
                    <button onClick={()=>{setShowMood(true);setMoodTab("partner");}} style={{width:"100%",background:T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"14px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",textAlign:"left"}}>
                      <div><div style={{fontSize:14,color:T.text,marginBottom:2}}>{T.partnerName}'s Mood</div><div style={{fontSize:10,color:T.t3}}>Translated · partner feed</div></div>
          ...
          {nav==="body"&&(
            <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
              <div style={{display:"flex",borderBottom:`1px solid ${T.b1}`,background:imp?"#000":T.s1,overflowX:"auto",flexShrink:0,position:"relative"}}>
                {T.practiceItems.map((lbl,i)=>(
                  <button key={i} onClick={()=>setBodyTab(i)} style={{flexShrink:0,padding:"10px 12px",background:"none",border:"none",borderBottom:`2px solid ${bodyTab===i?T.cBody:"transparent"}`,color:bodyTab===i?T.cBody:T.t3,fontSize:8,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",position:"relative"}}>
                    {lbl}
                    {lbl==="Nourish"&&Object.keys(cart).length>0&&(
                      <span style={{position:"absolute",top:4,right:2,width:12,height:12,borderRadius:"50%",background:T.gold,fontSize:6.5,color:"#000",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:"bold"}}>{Object.keys(cart).length}</span>
                    )}
                  </button>
                ))}
              </div>
              <div style={{flex:1,overflowY:"auto"}}>
                {/* Nourish — full implementation (same as old nourish tab) */}
                {bodyTab===0&&(
                  <div style={{padding:"14px"}}>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cNour,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Nourish</div>
                    {/* Sub-tabs within Nourish */}
                    <div style={{display:"flex",gap:1,marginBottom:12}}>
                      {["Plan","Grocery","Ceremony"].map((t,i)=>(
                        <button key={t} onClick={()=>setNourTab(["plan","grocery","ceremony"][i])} style={{flex:1,padding:"7px 0",background:nourTab===["plan","grocery","ceremony"][i]?T.s2:"transparent",border:`1px solid ${nourTab===["plan","grocery","ceremony"][i]?T.b2:T.b1}`,color:nourTab===["plan","grocery","ceremony"][i]?T.text:T.t3,fontSize:8.5,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",borderRadius:i===0?"5px 0 0 5px":i===2?"0 5px 5px 0":"0"}}>{t}</button>
                      ))}
                    </div>
                    {/* Dietary filter toggles */}
                    <div style={{display:"flex",gap:5,marginBottom:10,overflowX:"auto",paddingBottom:2}}>
                      {Object.entries({nutAllergy:"NF",gerd:"GERD",glutenFree:"GF",dairyFree:"DF"}).map(([k,label])=>(
                        <button key={k} onClick={()=>setDiet(d=>({...d,[k]:!d[k]}))} style={{flexShrink:0,padding:"4px 10px",background:diet[k]?T.glow:"transparent",border:`1px solid ${diet[k]?T.gold:T.b2}`,color:diet[k]?T.gold:T.t3,fontSize:8,letterSpacing:"0.1em",borderRadius:12,cursor:"pointer"}}>{label}: {diet[k]?"ON":"off"}</button>
                      ))}
                    </div>
                    {/* Nourish content placeholder pointing to old nourish implementation */}
                    <div style={{fontSize:9,color:T.t3,textAlign:"center",padding:"8px 0",marginBottom:8}}>Full meal plan, grocery list, and ceremony supplies below ↓</div>
                  </div>
                )}
                {/* Practice */}
                {bodyTab===1&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 20px",gap:12}}>
                    <Svg path={IC.star} size={40} color={T.cBody}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cBody,letterSpacing:"0.14em",textTransform:"uppercase"}}>{T.practiceLabel}</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>{T.practiceDesc}<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* Hydration */}
                {bodyTab===2&&(
                  <div style={{padding:"14px"}}>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cBody,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Hydration</div>
                    <div style={{background:T.s1,border:`1px solid #4a8bb844`,borderRadius:8,padding:"16px",marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
                        <div style={{fontFamily:COR,fontSize:36,color:T.text,lineHeight:1}}>{hydCur}</div>
                        <div style={{fontSize:10,color:T.t3}}>/ {T.hydT} oz goal</div>
                      </div>
                      <div style={{background:T.s3,borderRadius:4,height:7,marginBottom:10,overflow:"hidden"}}>
                        <div style={{width:`${hydPct}%`,height:"100%",background:hydPct>=100?"#18c48a":"#4a8bb8",transition:"width 0.3s"}}/>
                      </div>
                      <div style={{display:"flex",gap:6}}>
                        {[8,16,24].map(oz=><button key={oz} onClick={()=>{setHyd(h=>({...h,[prof]:Math.min(h[prof]+oz,T.hydT)}));showT(`+${oz} oz`);}} style={{flex:1,padding:"9px 0",background:T.s3,border:`1px solid ${T.b1}`,color:"#4a8bb8",fontSize:11,cursor:"pointer",borderRadius:5}}>+{oz}oz</button>)}
                      </div>
                    </div>
                    <div style={{fontSize:9,color:T.t3,textAlign:"center"}}>Log water before coffee. Daily target on Home tab.</div>
                  </div>
                )}
                {/* Sleep stub */}
                {bodyTab===3&&(
                  <div style={{padding:"14px"}}>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cBody,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Sleep</div>
                    {sw?(
                      <div style={{background:T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"14px",marginBottom:10}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                          <div style={{textAlign:"center"}}><div style={{fontSize:8.5,color:T.t3,marginBottom:2}}>WIND DOWN</div><div style={{fontFamily:COR,fontSize:18,color:T.text}}>{sw.wd}</div></div>
                          <div style={{textAlign:"center"}}><div style={{fontSize:8.5,color:T.t3,marginBottom:2}}>SLEEP</div><div style={{fontFamily:COR,fontSize:18,color:T.text}}>{sw.sl}</div></div>
                          <div style={{textAlign:"center"}}><div style={{fontSize:8.5,color:T.t3,marginBottom:2}}>WAKE</div><div style={{fontFamily:COR,fontSize:18,color:T.text}}>{sw.wk}</div></div>
                        </div>
                        <div style={{fontSize:9,color:T.t3,textAlign:"center"}}>{sw.note}</div>
                      </div>
                    ):(
                      <div style={{fontSize:10,color:T.t3,textAlign:"center",padding:"20px 0"}}>Set your shift schedule in Planner → Schedule to see sleep windows.</div>
                    )}
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"20px 0"}}>
                      <div style={{fontSize:9,color:T.t3,textAlign:"center"}}>Sleep tracking and quality scores coming in a future layer.</div>
                    </div>
                  </div>
                )}
              </div>
          ...
          {nav==="soul"&&(
            <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
              <div style={{display:"flex",borderBottom:`1px solid ${T.b1}`,background:imp?"#000":T.s1,overflowX:"auto",flexShrink:0}}>
                {T.soulItems.map((lbl,i)=>(
                  <button key={i} onClick={()=>setSoulTab(i)} style={{flexShrink:0,padding:"10px 12px",background:"none",border:"none",borderBottom:`2px solid ${soulTab===i?T.cSoul:"transparent"}`,color:soulTab===i?T.cSoul:T.t3,fontSize:8,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer"}}>{lbl}</button>
                ))}
              </div>
              <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
                {/* The Book */}
                {soulTab===0&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.star} size={40} color={T.cSoul}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase"}}>The Eight Who Carry the Fire</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>The holy book of the Sovereign Traditions<br/>Eight movements · All 64 deity-position combinations<br/><br/>Unlocks after the Rite is completed (Phase 5).</div>
                    <div style={{marginTop:8,padding:"8px 16px",background:T.glow,border:`1px solid ${T.gold}33`,borderRadius:20}}>
                      <div style={{fontSize:8.5,color:T.gold,letterSpacing:"0.12em",textTransform:"uppercase",textAlign:"center"}}>Phase 5 · Requires Rite Completion</div>
                    </div>
                  </div>
                )}
                {/* Rites */}
                {soulTab===1&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.star} size={40} color={T.cSoul}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase"}}>The Rites</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Ceremony · Rite of Passage<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* Partnership */}
                {soulTab===2&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.heart} size={40} color={T.cSoul}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase"}}>Partnership</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Communication translator · Concord manual<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* Holy Days */}
                {soulTab===3&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.star} size={40} color={T.cSoul}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase"}}>Holy Days</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Ten observances · Tradition calendar<br/>Coming in a future layer.</div>
                  </div>
                )}
                {/* Tending: Morning Quiet timer */}
                {!imp&&soulTab===5&&(
                  <div style={{padding:"0"}}>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Morning Quiet</div>
                    <div style={{background:T.s1,border:`1px solid ${T.cSoul}33`,borderRadius:8,padding:"20px",textAlign:"center",marginBottom:10}}>
                      {quietRunning&&quietSecs!==null?(
                        <>
                          <div style={{fontFamily:"monospace",fontSize:44,color:T.gold,lineHeight:1,marginBottom:8}}>{String(Math.floor(quietSecs/60)).padStart(2,"0")}:{String(quietSecs%60).padStart(2,"0")}</div>
                          <div style={{background:T.s3,borderRadius:4,height:5,marginBottom:12,overflow:"hidden"}}>
                            <div style={{width:`${(quietSecs/900)*100}%`,height:"100%",background:T.cSoul,transition:"width 1s linear"}}/>
                          </div>
                          <button onClick={()=>{setQuietRunning(false);setQuietSecs(null);}} style={{padding:"9px 24px",background:"transparent",border:`1px solid ${T.b2}`,color:T.t3,fontSize:9,letterSpacing:"0.1em",cursor:"pointer",borderRadius:4}}>Stop</button>
                        </>
                      ):(
                        <>
                          <div style={{fontFamily:COR,fontSize:36,color:T.t3,lineHeight:1,marginBottom:4}}>15:00</div>
                          <div style={{fontSize:9,color:T.t3,marginBottom:16}}>No screen. No sound. Just time.</div>
                          <button onClick={()=>{setQuietSecs(15*60);setQuietRunning(true);}} style={{padding:"11px 28px",background:T.cSoul,border:"none",color:"#fff",fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase",cursor:"pointer",borderRadius:5}}>Begin Quiet</button>
                          {done.quiet&&<div style={{fontSize:9,color:T.teal,marginTop:12}}>✓ Completed today</div>}
                        </>
                      )}
                    </div>
                    <div style={{fontFamily:COR,fontSize:13,color:T.t2,fontStyle:"italic",textAlign:"center",lineHeight:1.6,padding:"0 10px"}}>Twenty minutes of quiet first. The rest waits.</div>
                  </div>
                )}
                {/* Tending: Replenishment */}
                {!imp&&soulTab===4&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                    <Svg path={IC.heart} size={40} color={T.cSoul}/>
                    <div style={{fontFamily:COR,fontSize:20,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase"}}>Replenishment</div>
                    <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Two hours. Protected. Not negotiable.<br/>Log and schedule here. Coming in a future layer.</div>
                  </div>
                )}
                {/* Evening Inventory */}
                {((imp&&soulTab===4)||(!imp&&soulTab===6))&&(
                  <div>
                    <div style={{fontFamily:COR,fontSize:22,color:T.cSoul,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>Evening Inventory</div>
                    {(imp?[
                      "What did I build today that will still be standing in a year?",
                      "Where did I perform instead of decide?",
                      ...
                      <div key={i} style={{background:T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"13px",marginBottom:8}}>
                        <div style={{fontFamily:COR,fontSize:14,color:T.text,lineHeight:1.6,marginBottom:6}}>{q}</div>
                        <textarea placeholder="Write here..." style={{width:"100%",background:"transparent",border:"none",borderTop:`1px solid ${T.b1}`,color:T.t2,fontSize:11,fontFamily:COR,lineHeight:1.6,resize:"none",outline:"none",paddingTop:8,minHeight:48}}/>
                      </div>
                    ))}
                    <button onClick={()=>{setDone(d=>({...d,inv:true}));showT("Inventory complete");}} style={{width:"100%",padding:"11px 0",background:done.inv?T.tealBg:"transparent",border:`1px solid ${done.inv?T.teal:T.b2}`,color:done.inv?T.teal:T.t3,fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",borderRadius:5}}>{done.inv?"✓ Complete":"Mark complete"}</button>
                  </div>
                )}
              </div>
            </div>
          ...
          {nav==="planner"&&(
            <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
              ...
              {planTab==="alarms"&&(
                <div>
                  <div style={{fontSize:9,color:T.t3,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:10}}>Alarm Management · Native wiring in Layer 3</div>
                  {[
                    {l:"Wake Alarm",     t:sw?sw.wk:"--",   days:"Work days",      on:true},
                    {l:"Wind-Down",      t:sw?sw.wd:"--",   days:"Every day",      on:true},
                    {l:"Sleep Alarm",    t:sw?sw.sl:"--",   days:"Every day",      on:true},
                    {l:"Hydration",      t:"Every 90 min",  days:"7 AM – 9 PM",    on:true},
                    {l:"Batch Cook",     t:"7:00 PM",       days:"Night before",   on:true},
                  ].map((al,i)=>(
                    <div key={i} style={{background:T.s1,border:`1px solid ${T.b1}`,borderRadius:8,padding:"11px 13px",marginBottom:7,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div>
                        <div style={{fontSize:13,color:T.text,marginBottom:2}}>{al.l}</div>
                        <div style={{fontSize:9,color:T.t3}}>{al.t} · {al.days}</div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <div style={{width:28,height:16,borderRadius:8,background:al.on?T.teal:T.b2,position:"relative",cursor:"pointer"}}>
                          <div style={{position:"absolute",top:2,left:al.on?14:2,width:12,height:12,borderRadius:6,background:"#fff",transition:"left 0.2s"}}/>
                        </div>
                      </div>
                    </div>
                  ))}
                  {otVal&&(
                    <div style={{background:T.s1,border:`1px solid #c0704044`,borderRadius:8,padding:"11px 13px",marginBottom:7,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div>
                        <div style={{fontSize:13,color:"#c07040",marginBottom:2}}>Overtime Active — OT +{otVal}h</div>
                        <div style={{fontSize:9,color:T.t3}}>Tomorrow's wake alarm adjusted</div>
                      </div>
                      <button onClick={()=>{setOtVal(null);showT("Overtime cancelled");}} style={{background:"transparent",border:`1px solid ${T.b1}`,color:T.t3,fontSize:8,letterSpacing:"0.1em",padding:"4px 8px",cursor:"pointer",borderRadius:3}}>Cancel</button>
                    </div>
                  )}
                  <button onClick={()=>setShowAlarm(true)} style={{width:"100%",marginTop:6,padding:"10px 0",background:"transparent",border:`1px solid ${T.b1}`,color:T.t3,fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",borderRadius:5}}>Preview alarm screen →</button>
                </div>
              )}
              {planTab==="reminders"&&(
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",gap:12}}>
                  <Svg path={IC.bell} size={40} color={T.cPlan}/>
                  <div style={{fontFamily:COR,fontSize:20,color:T.cPlan,letterSpacing:"0.14em",textTransform:"uppercase"}}>Reminders</div>
                  <div style={{fontSize:10,color:T.t3,textAlign:"center",lineHeight:1.6}}>Custom one-off and recurring reminders<br/>Coming in a future layer.</div>
                </div>
              )}
                            {planTab==="schedule"&&(
                <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
        ...
        <div style={{height:60,background:imp?"#000":T.s1,borderTop:`1px solid ${T.b1}`,display:"flex",alignItems:"stretch",flexShrink:0}}>
          {NAV.map(tab=>{
            ...
            return <button key={tab.k} onClick={()=>goNav(tab.k)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2.5,background:"none",border:"none",borderTop:`2px solid ${active?tab.col:"transparent"}`,color:active?tab.col:T.t3+"66",cursor:"pointer",fontSize:7,letterSpacing:"0.08em",textTransform:"uppercase",padding:"6px 1px 3px",transition:"all 0.15s",minWidth:0,position:"relative"}}>
              <div style={{position:"relative"}}>
                <Svg path={tab.ic} size={17} color={active?tab.col:T.t3+"55"}/>
                {tab.dot&&!active&&<span style={{position:"absolute",top:-2,right:-2,width:6,height:6,borderRadius:"50%",background:tab.col,border:`1px solid ${imp?"#000":T.s1}`}}/>}
              </div>
              <span>{tab.l}</span>
            </button>;
          })}
        ...
        {/* OT bottom sheet */}
        {otSheet&&(
          <div style={{position:"absolute",inset:0,zIndex:50,display:"flex",alignItems:"flex-end"}} onClick={()=>setOtSheet(false)}>
            <div style={{width:"100%",background:T.s2,border:`1px solid ${T.b2}`,borderRadius:"16px 16px 0 0",padding:"16px 20px 28px"}} onClick={e=>e.stopPropagation()}>
              <div style={{fontSize:9,color:T.t3,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12,textAlign:"center"}}>Set Overtime</div>
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                {[1,2,4].map(h=>(
                  <button key={h} onClick={()=>{setOtVal(h);setOtSheet(false);showT(`OT +${h}h set`);}} style={{flex:1,padding:"12px 0",background:otVal===h?T.glow:"transparent",border:`1px solid ${otVal===h?T.gold:T.b2}`,color:otVal===h?T.gold:T.t2,fontSize:13,cursor:"pointer",borderRadius:6}}>+{h}h</button>
                ))}
              </div>
              {otVal&&<button onClick={()=>{setOtVal(null);setOtSheet(false);showT("OT cancelled");}} style={{width:"100%",padding:"10px 0",background:"transparent",border:`1px solid ${T.b1}`,color:T.t3,fontSize:8.5,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",borderRadius:5}}>Cancel overtime</button>}
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{marginTop:14,fontFamily:JOS,fontSize:8.5,color:"#333",letterSpacing:"0.2em",textTransform:"uppercase",textAlign:"center"}}>
        5 tabs: Home · Planner (Today/Calendar/Alarms/Reminders/Schedule) · Mind · Body · Soul
      </div>
    </div>
  );
}
